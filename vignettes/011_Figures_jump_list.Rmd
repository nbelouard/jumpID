...
# 1. select all diffusion points
diffusion <- setdiff(grid_data %>% filter(established == T), 
                        Jumps %>% select(year, latitude, longitude, established, DistToIntro)) %>% # remove jumps from grid data
...
for (jump in 1:length(Jumps$DistToIntro)){ #for each jump
  y = Jumps[jump,'year']
  diffusion_y <- diffusion %>% filter(year == y)
  pairwise_dist <- geosphere::distGeo(diffusion_y[,c('longitude','latitude')], Jumps[jump,c('longitude','latitude')])/1000
  Jumps$DistToFront[jump] = min(pairwise_dist) 
}
 
for (jump in 1:length(Jump_clusters$DistToIntro)){ #for each jump
  y = Jump_clusters[jump,'year']
  diffusion_y <- diffusion %>% filter(year == y)
...
for (jump in 1:length(Jumps_2021$DistToIntro)){ #for each jump
  ...
  testDiff <- diff %>% filter(year == Jumps_2021$year[jump]+1)
  pairwise_dist <- geosphere::distGeo(testDiff[,c('longitude','latitude')], Jumps_2021[jump,c('longitude','latitude')])/1000
  
  if (min(pairwise_dist) < 15){ Jumps_2021$secDiff[jump] = "enveloped" 
  } else {
